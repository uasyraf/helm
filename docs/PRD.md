...
| **2 — Multi-dev** ✓ shipped 2026-05-18 | HTTP transport (`helm serve --http`, bearer auth, `/health`), Turso embedded-replica sync (`HELM_SYNC_URL`/`HELM_SYNC_TOKEN` env or `.helm/config.json`), `helm init --team` config writer, BYOS Postgres data layer (pg-core schema, dynamic `pg` import, `openPgDb`/`openPgliteDb`). | `/health` returns 200; two-dev sync verified against local sqld in 248ms (PRD gate: 10s); Postgres data layer verified against pglite (open + bootstrap + CRUD). Tool-surface portability to pg-core schema tracked as follow-up story. |
